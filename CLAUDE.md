...
**Java 1.20.1 对等重建 · 全部五个阶段完成** — P1/P2（存档 v3、固定种子回归、浏览器/长稳/双客户端冒烟与 60 FPS 门禁；BlockState、TickScheduler、行为注册表、标签/战利品/工具等级/经验规则）、P3（内容补齐）、P4（音画与手感）、P5（多人权威化与发布质量：物品动作/背包状态/容器/连接状态权威化 + 部署文档与冒烟）全部完成，155+ 项回归测试。后续迭代围绕对等重建计划的持续打磨（见 `docs/parity-manifest.json` 的 partial 项）。开发顺序与验收口径以 `docs/DEVELOPMENT_ROADMAP.md` 顶部的"对等重建计划"和 `docs/parity-manifest.json` 为准。
