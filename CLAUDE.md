...
**Java 1.20.1 对等重建 · 阶段 3 完成** — P1/P2 全部完成（存档 v3、固定种子回归、浏览器/长稳/双客户端冒烟与 60 FPS 门禁；BlockState、TickScheduler、行为注册表、纯放置规划器、标签/战利品表/工具等级/经验规则数据化）。阶段 3（1.20.1 内容补齐）P3.1–P3.6 已全部完成：交互方块、配方书、状态效果/附魔（15 效果 / 20 附魔）、酿造全配方、工作站（切石机/制图台/织布机）、红石时序与组件。**下一步按路线图进入"音画与手感"**。开发顺序与验收口径以 `docs/DEVELOPMENT_ROADMAP.md` 顶部的"对等重建计划"和 `docs/parity-manifest.json` 为准。
