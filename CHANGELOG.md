...
- Add server-authoritative inventory and player state (P5.2): clients upload their simulated health/hunger/oxygen (clamped server-side) so server pushes never revert them; consumable use (food/potions) is validated and deducted by the server with inventory sync; bow and tool durability is deducted server-side on release and block break; death drops the full inventory into the world server-side and respawns the player.
- Add container authority and world persistence (P5.3): the server owns chest/barrel/hopper contents (client open + throttled upload, per-slot validation, echo), and the world state (chunks, metadata, time, weather, containers) is saved to `server-world.json` on shutdown and restored on boot (`WORLD_PATH` configurable).
- Add connection lifecycle UI (P5.4): `NetworkClient` tracks connecting/connected/disconnected with a status callback, the HUD shows an online badge, a disconnected overlay offers returning to the menu, and the multiplayer menu documents `ws://`/`wss://` address formats.
- Add deployment docs and deeper multiplayer smoke (P5.5): `docs/MULTIPLAYER.md` covers local/LAN/public `ws://` and `wss://` setups (including an nginx HTTPS reverse-proxy example and the `PORT`/`WORLD_PATH` configuration table); the server gains a `/setblock` command and the two-client smoke now verifies the server-authoritative block-edit round-trip alongside join and chat.
