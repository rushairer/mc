...
**Java 1.20.1 对等重建 · 阶段 5：多人权威化与发布质量** — P1/P2（存档 v3、固定种子回归、浏览器/长稳/双客户端冒烟与 60 FPS 门禁；BlockState、TickScheduler、行为注册表、标签/战利品/工具等级/经验规则）、P3（内容补齐）、P4（音画与手感）全部完成。阶段 5 进行中：P5.1 物品动作 C2S、P5.2 背包/玩家状态权威已完成，下一个 ⬜ 为 P5.3 容器/方块元数据/存档持久化。开发顺序与验收口径以 `docs/DEVELOPMENT_ROADMAP.md` 顶部的"对等重建计划"和 `docs/parity-manifest.json` 为准。
